...
function PriceChart({ selectedCrypto }) {
  const startDatePlaceholder = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
  ...
  return (
    <div>
      ...
      {isLoading ? <div>Loading... <span className="spinner-border spinner-border-sm"></span></div> : null}
